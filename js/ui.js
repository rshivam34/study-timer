...
function fillDD(){var cfg=D.getCfg();
/* Study dropdown uses exam-grouped subjects */
var exams=D.getExams();var studyHtml='<option value="">Subject...</option>';
...
var workEl=document.getElementById('mngWork');
if(workEl){
  workEl.innerHTML='<div class="manage-list" id="ml-Work" data-type="Work">'+cfg.workCategories.map(function(s,i){return'<span class="manage-tag" data-idx="'+i+'" data-name="'+esc(s)+'"><span class="drag-handle">☰</span>'+esc(s)+'<button class="manage-x" onclick="event.stopPropagation();App.rmCat(\'work\',\''+esc(s).replace(/'/g,"\\'")+'\')">✕</button></span>'}).join('')+'</div>';
  DRAG.init('ml-Work');
}
...
/* Exam-grouped subject dropdown HTML — use everywhere a subject <select> is rendered */
function examSubjectOptions(selected,includeAll){
  var exams=D.getExams();
  var h=includeAll!==false?'<option value="">All Subjects</option>':'<option value="">Select...</option>';
  exams.forEach(function(ex){
    h+='<optgroup label="'+esc(ex.name)+'">';
    (ex.subjects||[]).forEach(function(s){
      h+='<option'+(s===selected?' selected':'')+'>'+esc(s)+'</option>';
    });
    h+='</optgroup>';
  });
  return h;
}
/* Exam filter dropdown HTML — for filtering by whole exam */
function examFilterOptions(selected){
  var exams=D.getExams();
  var h='<option value="">All Exams</option>';
  exams.forEach(function(ex){
    h+='<option value="'+esc(ex.id)+'"'+(ex.id===selected?' selected':'')+'>'+esc(ex.name)+'</option>';
  });
  return h;
}
...
if(examId){
  /* Exam-based subject reorder */
  var exams=D.getExams();var ex=exams.find(function(e){return e.id===examId});
  if(ex){var fi=parseInt(dragEl.dataset.idx),ti=parseInt(over.dataset.idx);var item=ex.subjects.splice(fi,1)[0];ex.subjects.splice(ti,0,item);D.setExams(exams);var cfg=D.getCfg();cfg.studySubjects=D.getAllSubjects();D.setCfg(cfg)}
} else {
  var cfg=D.getCfg(),key=list.dataset.type==='Study'?'studySubjects':'workCategories';var fi=parseInt(dragEl.dataset.idx),ti=parseInt(over.dataset.idx);var item=cfg[key].splice(fi,1)[0];cfg[key].splice(ti,0,item);D.setCfg(cfg);
}
UI.renderManage();UI.fillDD();D.push()}dragEl=null}
