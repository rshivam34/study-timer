...
function forceRefresh(){UI.toast('Refreshing…');var p=D.isCloud()?D.sync():Promise.resolve();p.then(function(){return caches.keys()}).then(function(ks){return Promise.all(ks.map(function(k){return caches.delete(k)}))}).then(function(){if('serviceWorker' in navigator)return navigator.serviceWorker.getRegistrations().then(function(regs){return Promise.all(regs.map(function(r){return r.unregister()}))})}).then(function(){location.reload(true)}).catch(function(){location.reload(true)})}
return{openSelective:openSelective,preset:preset,execSelective:execSelective,freshStart:freshStart,clearCache:clearCache,forceRefresh:forceRefresh}})();
...
var bedtime=D.getBedForDate(D.todayKey())||D.getCfg().bedtime||23;
