...
var RESET=(function(){
function openSelective(){var t=D.todayKey();document.getElementById('resetFrom').value=t;document.getElementById('resetTo').value=t;document.getElementById('rstStudy').checked=true;document.getElementById('rstWork').checked=true;document.getElementById('rstRevisions').checked=false;document.getElementById('rstSyllabus').checked=false;document.getElementById('rstDeadlines').checked=false;document.getElementById('rstRecurring').checked=false;document.getElementById('rstPlans').checked=false;document.getElementById('rstTodos').checked=false;document.getElementById('rstJournal').checked=false;document.getElementById('resetModal').classList.remove('hidden')}
...
function freshStart(){if(!confirm('Delete ALL sessions, revisions, syllabus, deadlines, recurring, plans, to-dos & journal?\nSubjects, categories & settings will be kept.'))return;if(!confirm('LAST CHANCE. This cannot be undone!'))return;var d=D.getLocal();d.study={};d.work={};d.revisions=[];d.syllabus={};d.deadlines=[];d.recurring=[];d.recurringDone={};d.todos={study:[],work:[]};d.plans={};d.journal={};localStorage.removeItem('st3_planTemplates');D.saveLocal(d);UI.renderAll();D.push();UI.toast('Fresh start ✓')}
