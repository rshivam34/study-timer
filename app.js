...
function renderQuotes(){var all=getAllQuotes(),cfg=D.getCfg(),custom=cfg.customQuotes||[];var el=document.getElementById('quoteList');el.innerHTML=all.map(function(q,i){var isCustom=i>=QUOTES.length;return'<div class="quote-item"><span>"'+q+'"</span>'+(isCustom?'<button class="quote-rm" onclick="event.stopPropagation();App.rmQuote('+(i-QUOTES.length)+')">✕</button>':'')+'</div>'}).join('')}
...
document.getElementById('studySubjList').innerHTML=cats.map(function(s){return'<span class="manage-tag">'+s+'<button class="manage-x" onclick="App.rmCat(\'study\',\''+s+'\')">✕</button></span>'}).join('');document.getElementById('workSubjList').innerHTML=wCats.map(function(s){return'<span class="manage-tag">'+s+'<button class="manage-x" onclick="App.rmCat(\'work\',\''+s+'\')">✕</button></span>'}).join('');
