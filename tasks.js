...
h+='<div class="dl-card '+cls+'" onclick="DL.edit(\''+d.id+'\')"><div class="dl-left"><div class="dl-title">'+typeIco+' '+d.title+'</div><div class="dl-meta">'+UI.fdateFull(d.date)+' · tap to edit</div></div><div class="dl-countdown" style="color:'+cdCol+'">'+cd+'</div></div>'});el.innerHTML=h}
...
rd.forEach(function(r){h+='<div class="rc-today-card" style="border-color:rgba(248,113,113,.3)"><div><div class="rc-today-title" style="color:var(--red)">'+r.subj+': '+r.topic+'</div><div class="rc-today-type">Revision · '+r.diff+' · next: '+UI.fdate(r.nextDate)+'</div></div></div>'});
odls.forEach(function(d){h+='<div class="rc-today-card" style="border-color:rgba(248,113,113,.3)"><div><div class="rc-today-title" style="color:var(--red)">'+d.title+'</div><div class="rc-today-type">Deadline · overdue · '+UI.fdateFull(d.date)+'</div></div></div>'})}
...
items.forEach(function(it){if(it.type==='rc'){var isDone=i===0&&done.indexOf(it.data.id)!==-1;h+='<div class="rc-today-card'+(isDone?' done':'')+'"><div><div class="rc-today-title">🔄 '+it.data.title+'</div><div class="rc-today-type">'+it.data.rcType+' · '+it.data.pattern+(it.data.time?' · '+it.data.time:'')+'</div></div>';if(i===0&&!isDone)h+='<div class="rc-today-actions"><button class="b b-xs b-grn" onclick="REM.markDone(\''+it.data.id+'\')">✅</button><button class="b b-xs" onclick="REM.dismiss(\''+it.data.id+'\')">👁</button></div>';h+='</div>'}
else if(it.type==='dl'){var t=new Date(it.data.date),tStr=t.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});h+='<div class="rc-today-card" onclick="DL.edit(\''+it.data.id+'\')"><div><div class="rc-today-title">⏰ '+it.data.title+'</div><div class="rc-today-type">Deadline · '+it.data.type+' · '+tStr+' · tap to edit</div></div></div>'}
else{h+='<div class="rc-today-card"><div><div class="rc-today-title">🧠 '+it.data.topic+'</div><div class="rc-today-type">Revision · '+it.data.subj+' · '+it.data.diff+'</div></div></div>'}});h+='</div>'}
...
function fillTypeDD(selId){var cfg=D.getCfg(),types=cfg.recurringTypes||['Birthdays','Anniversaries','Habits','Bills','Custom'];document.getElementById(selId).innerHTML=types.map(function(t){return'<option>'+t+'</option>'}).join('')}
...
// Type list
document.getElementById('rcTypeList').innerHTML='<div class="manage-list">'+types.map(function(t){return'<span class="manage-tag" style="cursor:default">'+t+'<button class="manage-x" onclick="RECUR.removeType(\''+t+'\')">✕</button></span>'}).join('')+'</div>';
// Fill dropdowns
fillTypeDD('rcType');var filter=document.getElementById('rcFilter');var fv=filter.value;filter.innerHTML='<option value="">All Types</option>'+types.map(function(t){return'<option'+(t===fv?' selected':'')+'>'+t+'</option>'}).join('');
// Pattern opts init
